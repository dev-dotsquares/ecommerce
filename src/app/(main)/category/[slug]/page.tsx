...
export default function CategoryPage() {
  ...
  const sortOption = searchParams.get('sort') as string || 'popularity';
  const sortedProducts = [...filteredProducts].sort((a, b) => {
    switch (sortOption) {
      case 'price-asc': return a.price - b.price;
      case 'price-desc': return b.price - a.price;
      case 'newest': return new Date(b.id).getTime() - new Date(a.id).getTime(); // Assuming id correlates with creation time
  ...
  return (
    <div className="container py-8 md:py-12">
        <Breadcrumb items={[{label: 'Home', href: '/'}, {label: 'Categories', href: '/'}, {label: category.name}]} className="mb-6"/>
        <h1 className="text-3xl font-bold mb-2">{category.name}</h1>
        <p className="text-muted-foreground mb-8">Showing {filteredProducts.length} products</p>
